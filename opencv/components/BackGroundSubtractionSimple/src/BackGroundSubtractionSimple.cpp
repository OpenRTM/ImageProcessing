...
{	
  ...
  /* 閾値処理を行う */
  threshold( differenceImage, differenceImage, m_threshold_level, THRESHOLD_MAX_VALUE, cv::THRESH_BINARY );
...
void BackGroundSubtractionSimple::labDifference( void )
{
  ...
  /* 閾値処理を行う */
  threshold(sqrDifferenceImage, m_resultImage, m_threshold_level, THRESHOLD_MAX_VALUE, cv::THRESH_BINARY);
}
...
void BackGroundSubtractionSimple::grayScaleDifference( void )
{
  ...
  /* グレースケールから2値に変換する */
  threshold( m_resultImage, m_resultImage, m_threshold_level, THRESHOLD_MAX_VALUE, cv::THRESH_BINARY );
}
